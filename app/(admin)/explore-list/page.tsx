...
import { exploreColumns } from "@/app/data/tableColumns";
import { useRouter } from "next/navigation";
import ConfirmationDialog from "../components/ConfirmationDialog";
import useSnackbar from "@/hooks/useSnackbar";
import { TableCard, TableHeight } from "@/styles/admin.styled";
...
export default function Calendar() {
    ...
    useEffect(()=>{
        if(!data) return
        const rowsWithId  = data.map((item:FilterItem)=> ({
            id: item._id,
            ...item
        }))
        setRows(rowsWithId)
    ...
    const handleEdit = (id: string) => {
        router.push(`/explore-list/edit/${id}`)
    }
    ...
    const dialogMessage = selectedIds.length > 0
            ? `Are you sure want to delete ${selectedIds.length} explore events`
        : `Are you sure want to delete this explore event`
    ...
    return(
        <>
            <Grid container spacing={2}>
                <Grid size={12}>
                    <TableCard >
                        <CardHeader title="Explore List" action={
                            <Stack direction="row">
                                ...
                                <IconWithTooltip 
                                    title="Add" 
                                    icon={<AddIcon />} 
                                    onClick={() => router.push(`/explore-list/add`)}
                                />
                                <IconWithTooltip 
                                    title="Refresh" 
                                    icon={<RefreshIcon />} 
                        ...
                        />
                        <Divider />
                        <TableHeight height={160}>
                            <DataGrid 
                                rows={filteredRows ? filteredRows : rows}
                                columns={exploreColumns(handleEdit, handleDelete)}
                                checkboxSelection
                                rowHeight={80}
                                initialState={{
                                    pagination: {
